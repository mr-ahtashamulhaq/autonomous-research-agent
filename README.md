...
```bash
git clone https://github.com/your-username/autonomous-research-agent
...
Then update `BACKEND_URL` in `frontend/app.js`:
```js
const CONFIG = {
  BACKEND_URL: "https://your-api.railway.app",  // ← your deployed API
};
```
...
2. Add environment variables: `GROQ_API_KEY`, `TAVILY_API_KEY`
3. Set start command: `uvicorn api:app --host 0.0.0.0 --port $PORT`
4. Done - Railway auto-detects Python and installs requirements

### Backend → Render (alternative)

Same steps - Render also supports Python + free tier. Use start command:
```
...
Streams research progress and the final report as Server-Sent Events.
...
```
event: started
data: {"topic": "...", "total_steps": 5}

event: progress
data: {"node": "plan", "step": 1, "total": 5, "percent": 20, "label": "Generating search queries…"}

event: progress
data: {"node": "search", "step": 2, "total": 5, "percent": 40, "label": "Searching the web…"}

... (3 more progress events)
...
### `GET /health`
```json
{ "status": "ok", "service": "autonomous-research-agent" }
```

### `GET /docs`
Interactive Swagger UI - auto-generated by FastAPI. Try the API directly in the browser.

---

## 📊 Before vs After (Teaching Project → Resume Project)

| Aspect | Original (Notebook) | This Version (Production) |
|--------|--------------------|-----------------------------|
| **Entry point** | Runs on import | `argparse` CLI + FastAPI server |
| **API keys** | Hardcoded strings | `python-dotenv` + `.env` file |
| **Logging** | `print()` statements | Python `logging` module with timestamps |
| **Search depth** | 3 queries × 2 results = 6 URLs | 5 queries × 5 results = **25 candidates** |
| **Content extraction** | 4 URLs × 1,500 chars | 8 URLs × 3,000 chars = **4× more context** |
| **Plan prompt** | 1-line instruction | Role + angle-targeting + format rules |
| **Synthesise prompt** | "write a short report" | Structured sections + word target + citation rules |
| **Review prompt** | "remove unsupported claims" | Full fact-checking + numbered citations + confidence |
| **Streaming** | None | Server-Sent Events - live progress in browser |
| **Frontend** | None | Full Claude.com-themed HTML/CSS/JS app |
| **API docs** | None | Auto-generated Swagger UI at `/docs` |
| **Deployment** | Jupyter only | Vercel (frontend) + Railway/Render (backend) |
...
Agent orchestration & backend - handcrafted &nbsp;|&nbsp; Frontend - AI-generated
